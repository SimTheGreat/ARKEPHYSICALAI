...
import "./LiveViewDashboard.css";

function clamp01(value) {
  return Math.max(0, Math.min(1, value));
}

function LiveViewInlineSection({ children, onPhaseChange }) {
  ...
  async function autoCalibrateForEditingStation() {
    setCalibrating(true);
    try {
      const response = await axios.post("/api/vision/live/auto-calibrate", {
        station: editingStation,
        samples: 10,
      });
      await Promise.all([fetchLiveSettings(), fetchLiveStatus()]);
      const hsv = response?.data?.applied_hsv;
      if (hsv) {
        setInfo(
          `Auto-calibration applied for ${editingStation}: H(${hsv.lower_h}-${hsv.upper_h}) S>=${hsv.lower_s} V>=${hsv.lower_v}`
        );
      } else {
        setInfo(`Auto-calibration applied for ${editingStation}.`);
      }
      setError("");
    } catch (err) {
  ...
  return (
    <div className="live-view-root live-inline">
      <div className="live-inline-stack">
...
export default LiveViewInlineSection;
